...
/**
 * Mixed system of linear equalities and inequalities in modular arithmetic: Ax = b, A'x <= b' (mod p)
 * Requires p to be prime, thus defining a finite field F_p
 */
public class LinSystemModP extends AbstractConstraint {
    private int[][] A;
    private IntVar[] x;
    ...
    /**
     * Creates a "mixed system of linear modulo p" constraint, with p a prime number.
     * <p> This constraint holds iff
     * {@code Ae x = be (mod p) and Ai x <= bi (mod p)}.
     *
     * @param Ae the mexn matrix of coefficients for equalities
     * @param Ai the mixn matrix of coefficients for inequalities
     ...
     */
    public LinSystemModP(int[][] Ae, int[][] Ai, IntVar[] x, int[] be, int[] bi, int p) {
        super(x[0].getSolver(), x);
	setName("LinSystemModP");
	assert( nTuplesThreshold <= maxNbTuples );
	n = x.length;
	int me = be.length, mi = bi.length;
	m = me + mi;
        this.x = x;
	this.p = p;
	assert( p > 1 );
	...
	for (int i=0; i<me; i++) {
	    assert( Ae[i].length == n );
	    for (int j=0; j<nDisjuncts; j++) {
		this.A[i][nU+j] = be[i];
	    }
	    for (int j=0; j<nU; j++) {
		this.A[i][j] = Math.floorMod(Ae[i][unBounds[j]],p);
	    }
	    for (int j=nU; j<n; j++) { // subtract terms of bound variables from rhs
		for (int k=0; k<nDisjuncts; k++) {
		    this.A[i][nU+k] -= Ae[i][unBounds[j]]*x[unBounds[j]].min();
		}
	    }
	    for (int k=0; k<nDisjuncts; k++) {
	...
	for (int i=0; i<mi; i++) {
	    assert( Ai[i].length == n );
	    // fill rhs
	    nConsec /= bi[i]+1;
	    for (int l=0; l<period; l++) {
		for (int k=0; k<=bi[i]; k++) {
		    for (int j=0; j<nConsec; j++) {
			this.A[me+i][nU+l*nConsec*(bi[i]+1)+k*nConsec+j] = k;
		    }
		}
	    }
	    period *= bi[i]+1;
 	    for (int j=0; j<nU; j++) {
		this.A[me+i][j] = Math.floorMod(Ai[i][unBounds[j]],p);
	    }
	    for (int j=nU; j<n; j++) { // subtract terms of bound variables from rhs
		for (int k=0; k<nDisjuncts; k++) {
		    this.A[me+i][nU+k] -= Ai[i][unBounds[j]]*x[unBounds[j]].min();
		}
	    }
	    for (int k=0; k<nDisjuncts; k++) {
		this.A[me+i][nU+k] = Math.floorMod(this.A[me+i][nU+k],p);
	    }
	}
...
//    	    Log.constraint("likelihood = "+likelihood);
	    ...
	    /*
	    if ((nTuplesUB > nTuplesThreshold) && (nU > nNonparam+1)) // do nothing further until the number of tuples to enumerate is tractable or there is at most one unbound parametric var
		return;
	    */
