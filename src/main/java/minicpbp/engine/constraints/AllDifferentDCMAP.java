...
public class AllDifferentDCMAP extends AbstractConstraint {
	...
	// precompute gamma function up to n+1, to account for small floating-point
	// errors
	private void precompute_gamma(int n) {
		...
		for (int i = gamma_threshold + 1; i <= n + 1; i++) {
			// from n>gamma_threshold, Stirling's formula is a decent approximation of
			// factorial which will avoid intermediate overflow
...
//                System.out.println(var+" "+val+"; "+rSum+" " + rMax);
	...
	// compute the permanent of a real matrix through a simple adaptation of Heap's
	// Algorithm that generates all permutations
