...
public interface IntVar {
	...
	/**
	 * Asks that the closure is called whenever the domain of this variable is
	 * reduced to a single setValue
	 *
	 * @param f the closure
	 */
	...
	/**
	 * Asks that the closure is called whenever the max or min setValue of the
	 * domain of this variable changes
	 *
	 * @param f the closure
	 */
	...
	/**
	 * Asks that the closure is called whenever the domain change of this variable
	 * changes
	 *
	 * @param f the closure
	 */
	...
	/**
	 * Asks that {@link Constraint#propagate()} is called whenever the domain of
	 * this variable changes. We say that a <i>change</i> event occurs.
	 *
	 * @param c the constraint for which the {@link Constraint#propagate()} method
	...
	/**
	 * Asks that {@link Constraint#propagate()} is called whenever the domain of
	 * this variable is reduced to a singleton. In such a state the variable is bind
	 * and we say that a <i>bind</i> event occurs.
	 *
	 * @param c the constraint for which the {@link Constraint#propagate()} method
	 *          should be called on bind events of this variable.
	 */
	void propagateOnBind(Constraint c);

	/**
	 * Asks that {@link Constraint#propagate()} is called whenever the bound
	 * (maximum or minimum values) of the domain of this variable is changes. We say
	 * that a <i>bound change</i> event occurs in this case.
	 *
	 * @param c the constraint for which the {@link Constraint#propagate()} method
	 ...
	 * @exception InconsistencyException is thrown if the domain becomes empty
	 */
	void remove(int v);
	 ...
	 * @exception InconsistencyException is thrown if the value is not in the domain
	 */
	void assign(int v);
	...
	/**
	 * Returns the marginal of the specified value after cancelling out the local
	 * belief of a constraint.
	 *
	...
	/**
	 * Accumulates in the marginal of the specified value the local belief of a
	 * constraint.
	 *
	 * @param v the value, b the local belief
	 * @exception InconsistencyException is thrown if the value is not in the domain
	...
	/**
	 * @param b is True if the variable is a variable that can be branched on is
	 *          False if the variable is an auxiliary variable
	 */
	...
	/**
	 *
	 * @return weighted degree of this variable: sum of failure count of constraints
	 *         on this variable
	 */
	public int wDeg();
