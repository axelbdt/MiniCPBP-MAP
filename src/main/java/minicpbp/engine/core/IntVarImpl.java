...
/**
 * Implementation of a variable with a {@link SparseSetDomain}.
 */
public class IntVarImpl implements IntVar {
	...
	private StateStack<Constraint> constraints; // contains all constraints, allows us to get the failure count of all
												// constraints applied to the variable
	...
	/**
	 * Creates a variable with the elements {@code {0,...,n-1}} as initial domain.
	 *
	...
	/**
	 * Creates a variable with the elements {@code {min,...,max}} as initial domain.
	 *
	 * @param cp  the solver in which the variable is created
	...
	public void setMarginalWithDefault(int v, double m) {
		domain.setMarginalWithDefault(v, m);
	}
	...
	public int wDeg() {
		...
		Iterator<Constraint> iterator = constraints.iterator();
		while (iterator.hasNext()) {
			// in dom/wdeg all constraint's weigth are initialized to 1, that's why there is
			// a +1 for each constraint
	...
	public double sendMessage(int v, double b) {
		...
		assert domain.marginal(v) <= beliefRep.one() && domain.marginal(v) >= beliefRep.zero()
				: "domain.marginal(v) = " + domain.marginal(v);
