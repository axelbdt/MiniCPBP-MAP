...
public class SparseSetDomain implements IntDomain {
    ...
    private HashMap<Integer, ArrayList<Double>> impactValues; // a map containing the registered impact of an
    // assignement
    private Solver cp;
    private Belief beliefRep;
    static Random rand;

    private HashMap<Integer, Double> defaultBeliefValues; // values taken on reset
    ...
    public SparseSetDomain(Solver cp, int min, int max) {
        ...
        defaultBeliefValues = new HashMap<Integer, Double>();
    ...
    public void resetMarginals() {
        ...
        for (int j = 0; j < s; j++) {
            int v = domainValues[j];
            setMarginal(v, defaultBeliefValues.getOrDefault(v, beliefRep.one()));
        }
    }
    ...
    public double normalizedEntropy() {
        return entropy() / (Math.log((double) domain.size()));
    }
    ...
    public double impact() {
        ...
        double impact = 0.0;
        for (int j = 0; j < s; j++) {
            // System.out.println("value : " + domainValues[j] + " , impact : " +
            // impactOfValue(domainValues[j]));
